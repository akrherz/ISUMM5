...
char * name ;
{
    sym_nodeptr new_node(); 
    char **node_name() ;
    sym_nodeptr *node_next() ;
    return( symget( name, new_node, node_name, node_next, symtab, 1 ) ) ;
}
...
char * name ;
{
    ...
    char **node_name() ;
    sym_nodeptr *node_next() ;
    return( symget( name, new_node, node_name, node_next, symtab, 0 ) ) ;
}

sym_nodeptr
new_node()
{
    sym_nodeptr my_malloc(), p ;
    p = my_malloc( sizeof( struct sym_node ) ) ;
    p->name = NULL ;
    p->next = NULL ;
