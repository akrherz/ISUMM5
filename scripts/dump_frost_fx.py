...
def compute_times(nc):
    ...
    base = datetime.datetime.strptime(tm.units.replace("minutes since ", ""),
                                      '%Y-%m-%d %H:%M:%S')
    ret = []
    for t in tm[:]:
...
def main(argv):
    ...
    o.write(",".join(['station', 'model_run', 'forecast_time', 'model',
                      'temp_k']))
    o.write("\n")
    while now < ets:
        for model in ['bridget', 'metro']:
            fn = now.strftime(("/mesonet/share/frost/" + model + "/"
            ...
            for site, i, j in zip(sites, i_cross, j_cross):
                ...
                for k, tm in zip(tmpk, times):
                    o.write(",".join([site, now.strftime("%Y-%m-%d %H:%M"),
